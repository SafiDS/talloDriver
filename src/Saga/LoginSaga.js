  ...
  if (params["full_name"] == "") {
    ...
    if (response.status == 200) {
      if (response.data["statusCode"] == "SUCCESS") {
        ...
        navigateScreen(navigation, isUpdateProfile ? "Dashboard" : "Language");
...
export function* RiderLocationUpdate(action) {
  let params = {};
  const { rider_id, lat, lon, online_status, goto_home_status } =
    action.payload;
...
export function* RiderBooking(action) {
  ...
  if (response.data["statusCode"] == "SUCCESS") {
    ...
    yield put({ type: "RIDER_BOOKINGS", payload: response.data.bookings });
  } else {
    saveToStorage("StatusHistory", response.data["statusCode"]);
    console.log(response.data.statusMessage);
  }
...
export function* RiderLoginImage(action) {
  ...
  if (response.data["statusCode"] == "SUCCESS") {
    ...
    if (lat != "" && long != "") {
      ...
      navigation.navigate("RiderStatusOnline1", { lat: lat, lon: long });
    }
  } else {
    alert(response.data.statusMessage);
  }
